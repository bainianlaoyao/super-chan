...
键位与提交规则（明确优先级）：
- Enter：提交当前 textarea 内容作为一次 request。提交仅在以下条件满足时发生：补全面板未激活或补全未高亮项，且光标位于最后一行。
- Ctrl+Enter / Shift+Enter：在 textarea 内插入换行，不触发提交。
...
- 键盘导航：Tab / Shift+Tab 切换字段，Arrow 键在可选项内导航，Enter 在表单未被多行字段占用时可触发提交，Esc 取消并关闭 modal。
